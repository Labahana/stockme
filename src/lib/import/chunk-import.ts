...
import { pickField, parseInteger, parseDecimal } from "@/lib/import/csv";
import type { ImportResult } from "@/lib/import/stocky";
...
function mapPoStatus(raw: string): "draft" | "sent" | "partially_received" | "received" | "cancelled" {
  ...
  if (s.includes("sent") || s.includes("open") || s.includes("order") || s.includes("pending")) {
    return "sent";
  }
...
/** Import a chunk of supplier rows (streaming Stocky CSV). */
export async function importSupplierRows(
  ...
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const name = pickField(row, ["name", "supplier", "supplier_name", "company", "vendor"]);
    if (!name) {
      result.skipped++;
      result.errors.push(`Row ${rowOffset + i + 2}: missing supplier name`);
      continue;
    }
    ...
    const payload = {
      ...
      address: pickField(row, ["address", "street", "location", "bill_to_address"]) || null,
    ...
    if (existingId) {
      ...
      if (error) {
        result.errors.push(`Row ${rowOffset + i + 2}: ${error.message}`);
        result.skipped++;
      } else {
        result.updated++;
      }
...
/**
 * Import a chunk of PO line rows. Safe across chunks: existing POs get new
 * line items appended when the SKU is not already on the PO.
 */
export async function importPurchaseOrderRows(
  shopId: string,
  rows: Record<string, string>[],
  options: { createMissingSuppliers?: boolean; rowOffset?: number } = {},
): Promise<ImportResult> {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!poNumber) {
      result.skipped++;
      result.errors.push(`Row ${rowOffset + i + 2}: missing PO number`);
      continue;
    }
    ...
    if (!sku) {
      result.skipped++;
      result.errors.push(`Row ${rowOffset + i + 2} (${poNumber}): missing SKU`);
      continue;
    }
    ...
    if (!group) {
      group = {
        poNumber,
        supplierName: pickField(row, ["supplier", "supplier_name", "vendor", "vendor_name"]),
        ...
        expectedAt:
          pickField(row, ["expected_at", "expected_date", "expected delivery", "due_date"]) || null,
        lines: [],
      };
  ...
  for (const group of Array.from(groups.values())) {
    ...
    if (existingPo) {
      ...
      const have = new Set((existingLines ?? []).map((l) => l.variant_id));
      const toAdd = lineRows.filter((l) => !have.has(l.variant_id));
      if (toAdd.length === 0) {
        result.skipped++;
        continue;
      }
